...
export async function GET() {
  try {
    ...
    if (error) {
      console.error("QC data-quality query error:", error);
      return NextResponse.json({ error: "Failed to load employees" }, { status: 500 });
    }

    const { data: salaries } = await admin.from("hr_salary").select("employee_id");
    ...
    console.error("QC data-quality error:", err);
