 ...
 *
 * Historical rows (payslips, attendance, leave, ...) keep their
 * original employee_id — every roster query already filters on
...
export async function POST(request: NextRequest) {
  ...
  if (keep_id === merge_id) {
    return NextResponse.json(
      { error: "keep_id and merge_id must differ" },
      { status: 400 }
    );
  }
  ...
  if (Object.keys(backfill).length > 0) {
    ...
    await supabase.from("op_employees").update(backfill).eq("id", keep_id);
  }
  ...
  await supabase.from("hr_employee_timeline").insert({
    ...
    metadata: { merged_id: merge_id, backfilled: Object.keys(backfill).filter((k) => k !== "updated_at") },
  });
