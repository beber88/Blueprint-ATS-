...
export default function OffboardingPage() {
  ...
  const load = async () => {
    setLoading(true);
    const res = await fetch("/api/hr/onboarding?type=offboarding");
  ...
  return (
    <OpsPageShell
      title={t("hr_mgmt.offboarding.title")}
      subtitle={t("hr_mgmt.offboarding.subtitle")}
    >
      {/* KPIs */}
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12, marginBottom: 20 }}>
        <KpiCard label="Active" value={activeCount} accent="#EF4444" />
      ...
      {loading ? (
        ...
        <OpsCard>
          <p style={{ textAlign: "center", padding: 40, color: "var(--text-secondary)" }}>
            {t("hr_mgmt.offboarding.no_active")}
          </p>
        </OpsCard>
      ) : (
              ...
              <OpsCard key={cl.id}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                  ...
                  <span style={{
                    ...
                    background: cl.status === "completed" ? "#10B98120" : "#EF444420",
                    color: cl.status === "completed" ? "#10B981" : "#EF4444",
                  }}>
                    {cl.status}
                  </span>
                </div>
                ...
                <div style={{ marginBottom: 12 }}>
                  ...
                  <div style={{ height: 6, borderRadius: 3, background: "var(--border-light)", overflow: "hidden" }}>
                    <div style={{ height: "100%", width: `${pct}%`, borderRadius: 3, background: pct === 100 ? "#10B981" : "#EF4444", transition: "width 0.3s" }} />
                  </div>
                </div>
