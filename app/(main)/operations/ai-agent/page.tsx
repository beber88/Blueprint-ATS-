...
export default function OpsAgentPage() {
  ...
  const send = async (text?: string) => {
    ...
    setLoading(true);
    try {
      const res = await fetch("/api/operations/ai-agent/chat", {
        method: "POST",
  ...
  const suggestions = [
    t("operations.agent.suggest.overdue"),
    t("operations.agent.suggest.who_didnt_report"),
    t("operations.agent.suggest.top_risks"),
    t("operations.agent.suggest.ceo_briefing"),
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", height: "100%", maxWidth: 960, margin: "0 auto", padding: "16px 20px" }}>
      <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
        ...
        <div>
          <h1 style={{ margin: 0, fontSize: 20, fontWeight: 600 }}>{t("operations.agent.title")}</h1>
          <p style={{ margin: "2px 0 0 0", fontSize: 12, color: "var(--text-secondary)" }}>{t("operations.agent.subtitle")}</p>
        </div>
      </div>
      ...
      <div style={{ flex: 1, overflowY: "auto", background: "var(--bg-card)", border: "1px solid var(--border-light)", borderRadius: 12, padding: 16, marginBottom: 12 }}>
        {messages.length === 0 ? (
          <div style={{ textAlign: "center", padding: "32px 16px", color: "var(--text-secondary)" }}>
            ...
            <p style={{ marginTop: 12 }}>{t("operations.agent.ask_anything")}</p>
      ...
      <div style={{ display: "flex", gap: 8 }}>
        <textarea
          ...
          placeholder={t("operations.agent.placeholder")}
          rows={2}
